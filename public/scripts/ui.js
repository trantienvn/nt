...
Events.on('display-name', e => {
    ...
    $displayName.textContent = 'Tên bạn là ' + me.displayName;
...
class PeerUI {
    ...
    html() {
        return `
            <label class="column center" title="Nhấp để gửi tệp hoặc nhấp chuột phải để gửi văn bản">
...
class ReceiveTextDialog extends Dialog {
    ...
    async _onCopy() {
        ...
        Events.fire('notify-user', 'Đã sao chép vào clipboard');
    }
}
...
class Notifications {
    ...
    _requestPermission() {
        Notification.requestPermission(permission => {
            ...
            this._notify('Chia sẻ linh hoạt hơn nữa!');
    ...
    _messageNotification(message) {
        if (isURL(message)) {
            const notification = this._notify(message, 'Nhấp để mở liên kết');
            this._bind(notification, e => window.open(message, '_blank', null, true));
        } else {
            const notification = this._notify(message, 'Nhấp để sao chép văn bản');
            this._bind(notification, e => this._copyText(message, notification));
        }
    }

    _downloadNotification(message) {
        const notification = this._notify(message, 'Bấm chuột để tải xuống');
        if (!window.isDownloadSupported) return;
    ...
    _copyText(message, notification) {
        ...
        this._notify('Đã sao chép văn bản vào khay nhớ tạm');
    }
...
class NetworkStatusUI {
    ...
    _showOfflineMessage() {
        Events.fire('notify-user', 'Bạn đang offline');
    }

    _showOnlineMessage() {
        Events.fire('notify-user', 'Bạn đã trực tuyến trở lại');
    }
}
